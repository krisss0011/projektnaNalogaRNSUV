...
export class SupabaseService {
  ...
  private publicLinkOfImage(path: string) {
    return this.supabase.storage.from('images').getPublicUrl(path).data
      .publicUrl;
  }

  getImages(blogId: number) {
    return from(this.supabase.storage.from('images').list(blogId.toString()))
      .pipe(filter((res) => res.error === null && res.data != null))
      .pipe(
        map((res) =>
          res.data?.map((file) =>
            this.publicLinkOfImage(`${blogId}/${file.name}`),
          ),
        ),
      );
  }
  ...
  private deleteCorrectBlog(blogId: number, author: string) {
    return from(
      this.supabase.from('Blog').delete().eq('id', blogId).eq('author', author),
    );
  }
  deleteblog(blogId: number) {
    return this.currentAuthor().pipe(
      mergeMap((author) => this.deleteCorrectBlog(blogId, author!)),
    );
  }
  ...
  private postBlog(title: string, author: string, content?: string) {
    return from(
      this.supabase.from('Blog').insert({
        title,
        content,
        author,
      }),
  ...
  newBlog(title: string, content?: string) {
    return this.currentAuthor().pipe(
      mergeMap((author) => this.postBlog(title, author!, content)),
    );
  }

  uploadImage(blogId: number, file: File) {
    return from(
